...
void parse_csv(std::string filename, std::vector<Course> &courses)
{
  ...
  while (std::getline(f, t))
  {
    ...
    courses.push_back(Course{m[0], m[1], m[2]});
  }
}
...
{
  ofstream out_file("student_output/courses_offered.csv");
  int l = all_courses.size();
  ...
  for (int i = 0; i < l; ++i)
  {
    ...
    out_file << all_courses[i].title << "," << all_courses[i].number_of_units << "," << all_courses[i].quarter << '\n';
  }
}
...
{
  ofstream out_file("student_output/courses_not_offered.csv");
  out_file << "Title,Number of Units,Quarter" << '\n';
  for (auto &i : unlisted_courses)
  {
    ...
    out_file << i.title << "," << i.number_of_units << "," << i.quarter << '\n';
  }
}
