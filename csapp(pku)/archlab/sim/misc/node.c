...
node_ptr make_quote(char *qstring)
{
	...
	if (strstr(qstring, "#define"))
	{
		yyerror("#define is prohibited");
		return 0;
	}
	if (strstr(qstring, "constructor"))
	{
		yyerror("constructor is prohibited");
		return 0;
	}
...
node_ptr make_case(node_ptr arg1, node_ptr arg2)
{
	check_arg(arg1, 1);
	check_arg(arg2, 0);
	// 2021-10-25 LHY fix the bug by letting SBUB not assignable
	if (strcmp(arg2->sval, "SBUB") == 0)
	{
		yyerror("SBUB is not assignable signal");
		return 0;
	}
